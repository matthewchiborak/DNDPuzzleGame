...
ModelPlane::ModelPlane(float width, float height, std::string textureFilePath, bool isVert)
{
	this->width = width;
	this->height = height;

	if (isVert)
		makeVertPlane(textureFilePath);
	else
		makeHorzPlane(textureFilePath);
}
...
void ModelPlane::makeHorzPlane(std::string textureFilePath)
{
...
void ModelPlane::makeVertPlane(std::string textureFilePath)
{
	Texture texture((textureFilePath).c_str(), "diffuse", loadedTex.size());
	std::vector<Texture> textures;
	textures.push_back(texture);
